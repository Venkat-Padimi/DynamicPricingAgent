...
def render_cma_view(cma: Optional[CMAAnalysis], comparables: List[ComparableProperty]) -> None:
    """Render CMA summary statistics and interactive feature adjustment waterfall."""
    ...
    with c1:
        st.metric("Adjusted Median Price", f"${cma.adjusted_median_price:,.0f}")
    with c2:
        st.metric("Adjusted Mean Price", f"${cma.adjusted_mean_price:,.0f}")
    with c3:
        st.metric("Adjusted Price Spread", f"${cma.adjusted_price_low:,.0f} – ${cma.adjusted_price_high:,.0f}")
    with c4:
    ...
    comp_options = {f"{c.record.address} (Adj: ${c.adjusted_price:,.0f})": c for c in comparables}
        ...
        adj_records = [
            {
                ...
                "Applied Dollar Adjustment": f"${adj.adjustment_amount:+,.0f}",
